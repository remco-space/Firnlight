...
final class ContinuedAnalysisTask {
    ...
    func begin(onCancel: @escaping () -> Void) async {
        ...
        do {
            // submitTaskRequest(_:) is the async submission API introduced in
            // iOS 27, which the deployment floor (REQUIREMENTS.md's opening)
            // guarantees. It supersedes the synchronous submit(_:), deprecated
            // as of 27. Failure arrives as a thrown error, handled below.
